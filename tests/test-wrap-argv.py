...
def rewrap_arg(arg):
    ...
    while len(arg) > max_len:
        ...
        if split == -1:
            split = arg.rfind(" ", 0, max_len + 1)
        if split == -1:
            print("cannot find nice place to split '%s' below 80 chars" % arg, file=sys.stderr)
            split = max_len - 1
...
def rewrap(filename, in_place, check):
    ...
    if in_place:
        ...
        new = "".join(new_lines)
        if new != orig:
            diff = subprocess.Popen(["diff", "-u", filename, "-"], stdin=subprocess.PIPE)
            diff.communicate(input=new.encode('utf-8'))

            print("Incorrect line wrapping in $file", file=sys.stderr)
            print("Use test-wrap-argv.py to wrap test data files", file=sys.stderr)
            return False
    else:
        for line in new_lines:
            print(line)
