...
testQemuAgentUsers(const void *data)
{
    ...
    if (virTypedParamsGetUInt(params, nparams, "users.count", &count) < 0)
        goto cleanup;
    if (count != 2) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
    ...
    if (!username) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "Missing username");
        goto cleanup;
    }
    if (STRNEQ(username, "test")) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       "Expected user name 'test', got '%s'", username);
        goto cleanup;
    }
    ...
    if (STRNEQ(username, "test2")) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       "Expected user name 'test2', got '%s'", username);
        goto cleanup;
    }
    ...
    if (STRNEQ(domain, "DOMAIN")) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       "Expected domain 'DOMAIN', got '%s'", domain);
        goto cleanup;
    }
    ret = 0;
