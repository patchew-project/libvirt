...
def check_header(filename):
    ...
    if not ifdef.startswith("LIBVIRT_") or filename.find("libvirt_internal.h") != -1:
    ...
    with open(filename, "r") as fh:
        for line in fh:
            ...
            if state == STATE_COPYRIGHT_COMMENT:
                ...
                if line != "":
                    print("%s: missing blank line after copyright header" % filename, file=sys.stderr)
                    return True
                ...
                if line == "":
                    print("%s: too many blank lines after copyright header" % filename, file=sys.stderr)
                    return True
                elif re.match(r"""#ifndef %s$""" % ifdefpriv, line):
                    state = STATE_PRIV_ERROR
                else:
                    print("%s: missing '#ifndef %s'" % (filename, ifdefpriv), file=sys.stderr)
                    return True
            elif state == STATE_PRIV_ERROR:
                if re.match(r"""# error ".*"$""", line):
                    state = STATE_PRIV_END
                else:
                    print("%s: missing '# error ...priv allow...'" % filename, file=sys.stderr)
                    return True
            elif state == STATE_PRIV_END:
                if re.match(r"""#endif /\* %s \*/""" % ifdefpriv, line):
                    state = STATE_PRIV_BLANK
                else:
                    print("%s: missing '#endif /* %s */'" % (filename, ifdefpriv), file=sys.stderr)
                    return True
            elif state == STATE_PRIV_BLANK:
                if line != "":
                    print("%s: missing blank line after priv guard" % filename, file=sys.stderr)
                    return True
                state = STATE_GUARD_START
                ...
                if publicheader:
                    if re.match(r"""#ifndef %s$""" % ifdef, line):
                        ...
                        print("%s: missing '#ifndef %s'" % (filename, ifdef), file=sys.stderr)
                        return True
                else:
                    if re.match(r"""#pragma once""", line):
                        state = STATE_PRAGMA
                    else:
                        print("%s: missing '#pragma once' header guard" % filename, file=sys.stderr)
                        return True
            elif state == STATE_GUARD_DEFINE:
                if re.match(r"""# define %s$""" % ifdef, line):
                    state = STATE_GUARD_END
                else:
                    print("%s: missing '# define %s'" % (filename, ifdef), file=sys.stderr)
                    return True
                ...
                print("%s: unexpected content after '#endif /* %s */'" % (filename, ifdef), file=sys.stderr)
                return True
            else:
                print("%s: unexpected state $state" % filename, file=sys.stderr)
                return True


    if state == STATE_COPYRIGHT_COMMENT:
        print("%s: missing copyright comment" % filename, file=sys.stderr)
        return True
    elif state == STATE_COPYRIGHT_BLANK:
        print("%s: missing blank line after copyright header" % filename, file=sys.stderr)
        return True
        ...
        print("%s: missing blank line after priv header check" % filename, file=sys.stderr)
        return True
    elif state == STATE_GUARD_START:
        if publicheader:
            print("%s: missing '#ifndef %s'" % (filename, ifdef), file=sys.stderr)
        ...
        print("%s: missing '#endif /* %s */'" % (filename, ifdef), file=sys.stderr)
        return True
