    ...
    for line in fh:
        if line.find("/**") != -1:
            ...
            if line.find("* @aclfilter") != -1:
                ...
                m = re.match(r'''.*REMOTE_PROC_(.*)\s+=\s*\d+.*''', line)
                if m is not None:
    ...
    with open(filename, "r") as fh:
        ...
        for line in fh:
            ...
            if brace == 0:
                ...
                m = re.match(r'''.*\b(\w+)\(.*''', line)
                if m is not None:
                    maybefunc = m.group(1)
            elif brace > 0:
                ensureacl = re.match(r'''.*?(\w+)EnsureACL.*''', line)
                checkacl = re.match(r'''.*?(\w+)CheckACL.*''', line)
                stub = re.match(r'''.*?\b(\w+)\(.*''', line)
                if ensureacl is not None:
                    ...
                    if maybefunc is None:
                        print("%s:%d Unexpected check '%s' outside function" % (filename, lineno, func), file=sys.stderr)
                        errs = True
                    else:
                        if not maybefunc.lower().endswith(func.lower()):
                            print("%s:%d Mismatch check 'vir%sEnsureACL' for function '%s'" % (filename, lineno, func, maybefunc), file=sys.stderr)
                            errs = True
                    acls[maybefunc] = True
                elif checkacl:
                    # Record the fact that maybefunc contains an
            ...
            # an ACL function
            if intable:
                assign = re.match(r'''.*?\.(\w+)\s*=\s*(\w+),?.*''', line)
                if line.find("}") != -1:
                    intable = False
                    table = None
                    ...
                    if (impl != "NULL" and
                        ...
                        if (impl not in acls and
                            ...
                            print("%s:%d Missing ACL check in function '%s' for '%s'" % (filename, lineno, impl, api), file=sys.stderr)
                            errs = True

                        if api in filteredmap and impl not in aclfilters:
                            print("%s:%d Missing ACL filter in function '%s' for '%s'" % (filename, lineno, impl, api), file=sys.stderr)
                            errs = True
            else:
                m = re.match(r'''^(?:static\s+)?(vir(?:\w+)?Driver)\s+''', line)
                if m is not None:
                    name = m.group(1)
