...
vir_domain_checkpoint_class_init(virDomainCheckpointClass *klass)
{
    ...
    obj->dispose = virDomainCheckpointDispose;
...
static void virNetworkPortDispose(GObject *obj);
...
vir_node_device_class_init(virNodeDeviceClass *klass)
{
    ...
    obj->dispose = virNodeDeviceDispose;
...
static void virAdmClientDispose(GObject *obj);
...
vir_adm_client_class_init(virAdmClientClass *klass)
{
    ...
    obj->dispose = virAdmClientDispose;
}
 ...
 *
 * Unreferences the associated network object, which may also be
 * released if its ref count hits zero.
 */
static void
virNetworkPortDispose(GObject *obj)
{
    virNetworkPortPtr port = VIR_NETWORK_PORT(obj);

    g_clear_object(&port->net);

    G_OBJECT_CLASS(vir_network_port_parent_class)->dispose(obj);
}
 ...
 * @obj: the network port to release
 *
 * Unconditionally releases all memory associated with a network port.
 ...
 */
static void
virNodeDeviceDispose(GObject *obj)
{
    ...
    G_OBJECT_CLASS(vir_node_device_parent_class)->dispose(obj);
}
 ...
 * @obj: the node device to release
 *
 * Unconditionally releases all memory associated with a device.
 ...
 */
static void
virDomainCheckpointDispose(GObject *obj)
{
 ...
 * @obj: the domain checkpoint to release
 *
 * Unconditionally releases all memory associated with a checkpoint.
...
static void
virAdmClientDispose(GObject *obj)
{
