...
def load_file(fh):
    ...
    for line in fh:
        if re.match(r'''\s*enum\s+virNetMessageType\s*''', line):
            ...
            m = re.match(r'''^\s+VIR_NET_(\w+)\s*=\s*(\d+),?$''', line)
            if m is not None:
            ...
            m = re.match(r'''^\s+REMOTE_AUTH_(\w+)\s*=\s*(\d+),?$''', line)
            if m is not None:
                auth[m.group(2)] = m.group(1).lower()
        else:
            m = re.match(r'''^.*?(?:VIR_)?(\w+?)(?:_PROTOCOL)?_PROGRAM\s*=\s*0x([a-fA-F0-9]+)\s*;''', line)
            if m is not None:
            ...
            m = re.match(r'''^.*?(?:VIR_)?(\w+?)(?:_PROTOCOL)?_(?:PROGRAM|PROTOCOL)_VERSION\s*=\s*(\d+)\s*;''', line)
            if m is not None:
                funcs[m.group(1).lower()]["version"] = m.group(2)
                continue

            m = re.match(r'''^.*?(?:VIR_)?(\w+?)(?:_PROTOCOL)?_PROC_(.*?)\s+=\s+(\d+)''', line)
            if m is not None:
                funcs[m.group(1).lower()]["progs"].insert(int(m.group(3)), m.group(2).lower())
...
def genfunc(name, varname, types):
    ...
    print("  }")
    print("  if (verbose) {")
    print("      %sstr = %sstr . sprintf(\":%%d\", %s)" % (varname, varname, varname))
    print("  }")
...
for prog in sorted(funcs.keys()):
    ...
    print("  %s (program == %s && version == %s) {" % (cond, funcs[prog]["id"], funcs[prog]["version"]))
