                    ...
                    if (*data) {
                        ...
                        if (STRPREFIX(str, "pnode")) {
                            ...
                            if (!(token = virStringSplitCount(tmp, ",", 0, &ndistances)) ||
                                (virDomainNumaSetNodeDistanceCount(numa, vnodeCnt, ndistances) != ndistances))
                                goto cleanup;
    ...
    virDomainNumaPtr numa, size_t node, size_t nr_nodes)
{
    ...
    for (i = 0; i < nr_nodes; i++) {
        size_t distance = virDomainNumaGetNodeDistance(numa, node, i);

        /*
         * Present the configured distance value. If not
         * available set 10 for local or 21 for remote nodes.
         */
        if (!distance)
            distance = (node == i) ? 10 : 21;
        virBufferAsprintf(&buf, "%zu", distance);
        if ((nr_nodes - i) > 1)
            virBufferAddLit(&buf, ",");
    }
    xenFormatXLVnode(numaVnode, &buf);
