...
typedef struct _virDomainNumaLatency virDomainNumaLatency;
typedef virDomainNumaLatency *virDomainNumaLatencyPtr;
...
struct _virDomainNuma {
    ...
    struct _virDomainNumaLatency {
        virDomainNumaLatencyType type;  /* whether structure describes latency
                                           or bandwidth */
                         ...
                         xmlXPathContextPtr ctxt)
{
    ...
    VIR_FREE(nodes);
    if ((n = virXPathNodeSet("./cpu/numa[1]/latencies[1]/latency|"
                             "./cpu/numa[1]/latencies[1]/bandwidth", ctxt, &nodes)) < 0)
        goto cleanup;

    def->latencies = g_new0(virDomainNumaLatency, n);
    for (i = 0; i < n; i++) {
        ...
        if (virXMLNodeNameEqual(nodes[i], "latency")) {
            type = VIR_DOMAIN_NUMA_LATENCY_TYPE_LATENCY;

            if (!(tmp = virXMLPropString(nodes[i], "value"))) {
                virReportError(VIR_ERR_XML_ERROR, "%s",
                               _("Missing 'value' attribute in NUMA latencies"));
                goto cleanup;
            }

            if (virStrToLong_ullp(tmp, NULL, 10, &value) < 0) {
                virReportError(VIR_ERR_XML_ERROR, "%s",
                               _("Invalid 'value' attribute in NUMA latencies"));
                goto cleanup;
            }
            VIR_FREE(tmp);
        } else if (virXMLNodeNameEqual(nodes[i], "bandwidth")) {
            VIR_XPATH_NODE_AUTORESTORE(ctxt);
            type = VIR_DOMAIN_NUMA_LATENCY_TYPE_BANDWIDTH;
        ...
        if (!(tmp = virXMLPropString(nodes[i], "initiator"))) {
            virReportError(VIR_ERR_XML_ERROR, "%s",
                           _("Missing 'initiator' attribute in NUMA latencies"));
            goto cleanup;
        }

        if (virStrToLong_uip(tmp, NULL, 10, &initiator) < 0) {
            virReportError(VIR_ERR_XML_ERROR, "%s",
                           _("Invalid 'initiator' attribute in NUMA latencies"));
            goto cleanup;
        }
        VIR_FREE(tmp);
        ...
        if (!(tmp = virXMLPropString(nodes[i], "target"))) {
            virReportError(VIR_ERR_XML_ERROR, "%s",
                           _("Missing 'target' attribute in NUMA latencies"));
            goto cleanup;
        }

        if (virStrToLong_uip(tmp, NULL, 10, &target) < 0) {
            virReportError(VIR_ERR_XML_ERROR, "%s",
                           _("Invalid 'target' attribute in NUMA latencies"));
            goto cleanup;
        }
        VIR_FREE(tmp);
        ...
        if ((tmp = virXMLPropString(nodes[i], "cache"))) {
            if (virStrToLong_uip(tmp, NULL, 10, &cache) < 0 ||
                cache == 0) {
                virReportError(VIR_ERR_XML_ERROR, "%s",
                               _("Invalid 'cache' attribute in NUMA latencies"));
                goto cleanup;
            }
        }
        VIR_FREE(tmp);
        ...
        if (!(tmp = virXMLPropString(nodes[i], "type"))) {
            virReportError(VIR_ERR_XML_ERROR, "%s",
                           _("Missing 'type' attribute in NUMA latencies"));
            goto cleanup;
        }

        if ((accessType = virDomainMemoryLatencyTypeFromString(tmp)) <= 0) {
            virReportError(VIR_ERR_XML_ERROR, "%s",
                           _("Invalid 'type' attribute in NUMA latencies"));
            goto cleanup;
        }
        VIR_FREE(tmp);

        def->latencies[i] = (virDomainNumaLatency) {type, initiator, target,
                          ...
                          virDomainNumaPtr def)
{
    ...
    if (def->nlatencies) {
        virBufferAddLit(buf, "<latencies>\n");
        virBufferAdjustIndent(buf, 2);
    }

    for (i = 0; i < def->nlatencies; i++) {
        virDomainNumaLatencyPtr l = &def->latencies[i];
        ...
        if (l->type == VIR_DOMAIN_NUMA_LATENCY_TYPE_BANDWIDTH)
            virBufferAddLit(buf, " unit='KiB'");
        virBufferAddLit(buf, "/>\n");
    }
...
virDomainNumaDefValidate(const virDomainNuma *def)
{
    ...
    for (i = 0; i < def->nlatencies; i++) {
        const virDomainNumaLatency *l = &def->latencies[i];
        ...
        for (j = 0; j < i; j++) {
            virDomainNumaLatencyPtr ll = &def->latencies[j];
