                        ...
                        unsigned int flags)
{
    ...
    if (flags & VIR_DOMAIN_BACKUP_PARSE_INTERNAL) {
        g_autofree char *tmp = virXMLPropString(ctxt->node, "id");
                             ...
                             bool internal)
{
    ...
    return virXMLFormatElement(buf, "disk", &attrBuf, &childBuf);
}
                         ...
                         bool internal)
{
    ...
    g_auto(virBuffer) disksChildBuf = VIR_BUFFER_INITIALIZER;
    size_t i;

    virBufferSetChildIndent(&childBuf, buf);
    virBufferSetChildIndent(&disksChildBuf, &childBuf);

    virBufferAsprintf(&attrBuf, " mode='%s'", virDomainBackupTypeToString(def->type));
    virBufferAsprintf(&attrBuf, " id='%d'", def->id);
    ...
    if (virXMLFormatElement(&childBuf, "server", &serverAttrBuf, NULL) < 0)
        return -1;
    ...
    if (virXMLFormatElement(&childBuf, "disks", NULL, &disksChildBuf) < 0)
        return -1;

    if (virXMLFormatElement(buf, "domainbackup", &attrBuf, &childBuf) < 0)
        return -1;
