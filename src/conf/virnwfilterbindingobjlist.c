/*
 * virnwfilterbindingobjlist.c: binding objects list utilities
 *
...
{
    if (virHashAddEntry(bindings->objs, binding->def->portdevname, binding) < 0)
        return -1;
                                   ...
                                   virNWFilterBindingDefPtr def)
{
    ...
    if ((binding = virNWFilterBindingObjListFindByPortDevLocked(bindings, def->portdevname))) {
...
virNWFilterBindingObjListRemoveLocked(virNWFilterBindingObjListPtr bindings,
                                      virNWFilterBindingObjPtr binding)
{
    virHashRemoveEntry(bindings->objs, binding->def->portdevname);
}
 ...
 *       @binding was successfully added to both the bindings->objs
                                    ...
                                    const char *name)
{
    ...
    if (virHashLookup(bindings->objs, obj->def->portdevname) != NULL) {
                                ...
                                virNWFilterBindingObjListACLFilter filter)
{
    ...
    while (i < *nbindings) {
         ...
         */
        if (binding->removing ||
            (filter && !filter(conn, binding->def))) {
            virObjectUnlock(binding);
            virObjectUnref(binding);
                                ...
                                virNWFilterBindingObjListACLFilter filter)
{
    ...
    if (virNWFilterBindingObjListCollect(bindings, conn, &bindingobjs, &nbindings, filter) < 0)
        return -1;
    ...
    if (bindinglist) {
        ...
        for (i = 0; i < nbindings; i++) {
            ...
            virObjectLock(binding);
            (*bindinglist)[i] = virGetNWFilterBinding(conn, binding->def->portdevname, binding->def->filter);
            virObjectUnlock(binding);
