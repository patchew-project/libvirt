...
virSystemdHasCreateMachine(void)
{
    ...
    val = virAtomicIntGet(&virSystemdCreateMachineCachedValue);
    if (val != -1)
        return val;

    if ((ret = virDBusIsServiceEnabled("org.freedesktop.machine1")) < 0) {
        if (ret == -2)
            virAtomicIntSet(&virSystemdCreateMachineCachedValue, -2);
        return ret;
    }

    if ((ret = virDBusIsServiceRegistered("org.freedesktop.systemd1")) == -1)
        return ret;
    virAtomicIntSet(&virSystemdCreateMachineCachedValue, ret);
    return ret;
}
