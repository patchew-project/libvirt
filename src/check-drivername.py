        ...
        for line in fh:
            ...
            if m is not None:
                ...
                if sym not in symbols:
                    print("Driver method name %s doesn't match public API name" % drv)
                continue

            m = re.match(r'''^\*(vir\w+)\s*\)''', line)
            if m is not None:
                name = m.group(1)
            ...
            m = re.match(r'''^\s*(virDrv\w+)\s+(\w+);\s*''', line)
            if m is not None:
                drv = m.group(1)
                field = m.group(2)
                ...
                if tmp != field:
                    print("Driver struct field %s should be named %s" % (field, tmp))
                    status = 1
