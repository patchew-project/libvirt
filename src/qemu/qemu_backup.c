                                     ...
                                     virHashTablePtr blockNamedNodeData)
{
    ...
    if (qemuBlockGetBitmapMergeActions(backingChain, NULL, targetsrc,
                                       ...
                                       &tmpactions, &tmpallocbitmapnodes,
    ...
    if (allocationbitmapnodes) {
        *allocationbitmapnodes = g_slist_concat(*allocationbitmapnodes, tmpallocbitmapnodes);
        tmpallocbitmapnodes = NULL;
    }
                             ...
                             virQEMUDriverConfigPtr cfg)
{
    ...
    if (dd->backupdisk->incremental) {
        ...
        if (qemuBackupDiskPrepareOneBitmaps(dd, actions, allocationbitmapnodes,
                                            blockNamedNodeData) < 0)
            return -1;
    }
                ...
                unsigned int flags)
{
    ...
    if ((ndd = qemuBackupDiskPrepareData(vm, def, blockNamedNodeData, actions,
                                         &allocationbitmapnodes, cfg, &dd)) <= 0) {
        if (ndd == 0) {
    ...
    if (qemuBlockBitmapTemporaryAdd(vm, blockNamedNodeData, &allocationbitmapnodes,
                                    QEMU_ASYNC_JOB_BACKUP) < 0)
        goto endjob;
 ...
 endjob:
    ...
    qemuBlockBitmapTemporaryRemove(vm, allocationbitmapnodes, QEMU_ASYNC_JOB_BACKUP);
