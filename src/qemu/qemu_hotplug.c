                           ...
                           virDomainDeviceInfoPtr info)
{
    ...
    if (virAsprintf(&zpciAlias, "zpci%d", info->addr.pci.zpci->zpci_uid) < 0)
        goto cleanup;
                                     ...
                                     virDomainControllerDefPtr controller)
{
    ...
    if (qemuDomainAttachExtensionDevice(priv->mon, &controller->info,
                                        priv->qemuCaps) < 0) {
                            ...
                            virDomainVsockDefPtr vsock)
{
    ...
    if (qemuDomainAttachExtensionDevice(priv->mon, &vsock->info, priv->qemuCaps) < 0)
        goto exit_monitor;
                                     ...
                                     bool async)
{
    ...
    if (detach->type == VIR_DOMAIN_CONTROLLER_TYPE_PCI &&
        qemuDomainDetachExtensionDevice(priv->mon, &detach->info, priv->qemuCaps)) {
        ignore_value(qemuDomainObjExitMonitor(driver, vm));
        goto cleanup;
    }
