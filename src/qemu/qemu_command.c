                           ...
                           virQEMUCapsPtr qemuCaps)
{
    ...
    if (virBitmapLastSetBit(revmap) &&
        !virQEMUCapsGet(qemuCaps, QEMU_CAPS_VIRTIO_DISABLE_LEGACY)) {
        virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
    ...
    if (virBitmapIsBitSet(revmap, VIR_DOMAIN_VIRTIO_REVISION_0_9))
        virBufferAddLit(buf, "off");
    else
        virBufferAddLit(buf, "on");

    virBufferAddLit(buf, ",disable-modern=");
    if (virBitmapIsBitSet(revmap, VIR_DOMAIN_VIRTIO_REVISION_1_0))
        virBufferAddLit(buf, "off");
    else
        virBufferAddLit(buf, "on");
    return 0;
                   ...
                   virQEMUCapsPtr qemuCaps)
{
    ...
    if (usingVirtio &&
        qemuBuildVirtioRevisionStr(&buf, net->virtio_rev, qemuCaps) < 0)
        goto error;
