                                        ...
                                        virCPUDefPtr *baseline)
{
    ...
    *baseline = NULL;
    ...
    if (!(*baseline = virQEMUCapsCPUModelInfoToCPUDef(model_baseline, migratable)))
        goto cleanup;
                                 ...
                                 unsigned int flags)
{
    ...
    if (useLibvirt) {
        ...
        if ((qemuConnectBaselineHypervisorCPUViaQEMU(proc->mon, cpus, &cpu) < 0) || !cpu)
            goto cleanup;
    ...
    if (flags & VIR_CONNECT_BASELINE_CPU_EXPAND_FEATURES) {
        if (useLibvirt && virCPUExpandFeatures(arch, cpu) < 0) {
            ...
            if (qemuMonitorGetCPUModelExpansion(proc->mon,
                                                ...
                                                modelInfo, migratable, &expansion) < 0)
                goto cleanup;
