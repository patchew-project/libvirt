...
#define virCheckNetworkReturn(obj, retval) \
    do { \
        ...
        if (!G_IS_OBJECT(_net) || !(G_OBJECT_TYPE(_net) == VIR_TYPE_NETWORK) || \
...
#define virCheckNetworkPortReturn(obj, retval) \
    do { \
        ...
        if (!G_IS_OBJECT(_port) || !(G_OBJECT_TYPE(_port) == VIR_TYPE_NETWORK_PORT) || \
            !G_IS_OBJECT(_port->net) || !(G_OBJECT_TYPE(_port->net) == VIR_TYPE_NETWORK)) { \
            virReportErrorHelper(VIR_FROM_NETWORK, \
...
#define virCheckAdmClientReturn(obj, retval) \
    do { \
        ...
        if (!G_IS_OBJECT(_clt) || !(G_OBJECT_TYPE(_clt) == VIR_TYPE_ADM_CLIENT) || \
